...
static inline void filterEdge_c(const void* _prev2, const void* _prev, const void* _cur, const void* _next, const void* _next2, void* _dst, const int width, const int positiveStride, const int negativeStride, const int stride2, const int step, const int peak) noexcept
{
...
static inline void filterLine_c(const void* _prev2, const void* _prev, const void* _cur, const void* _next, const void* _next2, void* _dst, const int width, const int stride, const int stride2, const int stride3, const int stride4, const int step, const int peak) noexcept
{
    const pixel_t* prev2 = reinterpret_cast<const pixel_t*>(_prev2);
    const pixel_t* prev = reinterpret_cast<const pixel_t*>(_prev);
    const pixel_t* cur = reinterpret_cast<const pixel_t*>(_cur);
...
class BWDIF : public GenericVideoFilter
{
    ...
    template<typename pixel_t>
    void filter(PVideoFrame& prevFrame, PVideoFrame& curFrame, PVideoFrame& nextFrame, PVideoFrame& dstFrame, const int field, const BWDIF* const __restrict, IScriptEnvironment* env) noexcept;

public:
    BWDIF(PClip _child, int field, int opt, IScriptEnvironment* env);
...
template<typename pixel_t>
void BWDIF::filter(PVideoFrame& prevFrame, PVideoFrame& curFrame, PVideoFrame& nextFrame, PVideoFrame& dstFrame, const int field, const BWDIF* const __restrict, IScriptEnvironment* env) noexcept
{
    ...
    for (int i = 0; i < planecount; i++)
    {
        ...
        for (int y = field; y < height; y += 2) {
            if ((y < 4) || (y + 5 > height)) {
                if ((y < 2) || (y + 3 > height))
                    filterEdgeWithoutSpat(prev2, prev, cur, next, next2, dst, width,
                        y + 1 < height ? stride : -stride,
                        y > 0 ? -stride : stride,
                        stride * 2,
                        edgeStep, peak);
                else
                    filterEdgeWithSpat(prev2, prev, cur, next, next2, dst, width,
                        y + 1 < height ? stride : -stride,
                        y > 0 ? -stride : stride,
                        stride * 2,
                        edgeStep, peak);
...
BWDIF::BWDIF(PClip _child, int field, int opt, IScriptEnvironment* env)
    : GenericVideoFilter(_child), field_(field), opt_(opt)
{
    if (vi.height < 4)
...
PVideoFrame __stdcall BWDIF::GetFrame(int n, IScriptEnvironment* env)
{
    ...
    switch (vi.ComponentSize())
    {
        case 1: filter<uint8_t>(prev, cur, next, dst, field, 0, env); break;
        case 2: filter<uint16_t>(prev, cur, next, dst, field, 0, env); break;
        default: filter<float>(prev, cur, next, dst, field, 0, env); break;
    }
...
const char* __stdcall AvisynthPluginInit3(IScriptEnvironment * env, const AVS_Linkage* const vectors)
{
    ...
    env->AddFunction("BWDIF", "c[field]i[opt]i", Create_BWDIF, 0);
